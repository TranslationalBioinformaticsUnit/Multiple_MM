...
table_norm <-  read.table("C:/Users/transbio/Desktop/MM_analysis/human_rm_IG/Data/norm_NO_IG.txt",  sep="\t", dec=".", row.names=1, check.names = FALSE)
...
#plot
png("heatmap_sig_genes_MM_Control.png", width=1000, height=1000)
...
#plot
png("heatmap_sig_genes_MM_Control_short_negative.png", width=1000, height=1000)
Heatmap(data_to_plot2, name = "DEGs between MM and Control",
        top_annotation = ha1, 
        show_column_names = FALSE, show_row_names = FALSE, row_names_gp = gpar(fontsize = 6), 
        cluster_rows = TRUE, cluster_columns = TRUE, row_gap = unit(3, "mm"), use_raster=TRUE)
...
# >>> Y ploteo:
pdf("mouse_sig_in_human_ordered.pdf")
...
ref_genes <- unique(anno_human$ensembl_gene_id[common_ensembl])
ref_genes_intersection<-ref_genes[ref_genes%in%list_sig_genes_MM]
...
row_ha <- rowAnnotation(Mouse = as.factor(direction_info), col=list(Mouse=c("-1"="blue","0"="gray","1"="red")))
# >>> Y ploteo:
pdf("mouse_sig_in_human_rownames.pdf")
draw(Heatmap(data_to_plot2, name = "mouse signature in human data",
             top_annotation = ha1, right_annotation = row_ha,
             show_column_names = FALSE, show_row_names = TRUE, row_names_gp = gpar(fontsize = 6),
             cluster_rows = TRUE, cluster_columns = TRUE, row_gap = unit(3, "mm"), use_raster=TRUE))
dev.off()
